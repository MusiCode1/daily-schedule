...
פיתוח נרחב של ממשק ניהול הקיוסק: חלוקה לקומפוננטות, לחצן ריענון בפאנל המידע, קומפוננטת פעולות מכשיר מלאה, dropdown מותאם עם אייקוני אפליקציות ורשימת אחרונות.

#### מה בוצע?

**1. שיפורי UX לדף הניהול**

- הודעות פידבק הוסרו מתוך ה-scroll-area והפכו ל-toast צף `position: fixed` בתחתית המסך — לא מזיזות את הפריסה
- לחצן ריענון 🔄 הועבר מה-header לתוך `DeviceInfoPanel` (ליד שם המכשיר)
- ספינר טעינה ראשונית (`initialized` state) — מונע הבהוב בין לוגין לניהול
- לחצן "התנתק" נשאר לבד ב-header (נוקה לחצן הריענון)

**2. קומפוננטת `DeviceInfoPanel.svelte`**

- נוספו props: `onRefresh: () => void`, `isRefreshing: boolean`
- לחצן 🔄/⏳ ב-`.info-row` שמפעיל רענון ללא מעבר למסך לוגין

**3. קומפוננטת `ActionsPanel.svelte` (חדשה)**

- טאגל מסך — דינמי לפי `deviceInfo.screenOn` (💡 הדלק / 🌑 כבה)
- קדמה/רקע — שני כפתורים להעברת Fully לקדמה/רקע
- פתח כתובת בית — `loadStartUrl`
- פתח URL — input עם תמיכת Enter
- הפעל אפליקציה — dropdown מותאם + רשימת אחרונות

**4. קומפוננטת `AppDropdown.svelte` (חדשה)**

- Dropdown מותאם (לא native `<select>`) — מאפשר הצגת אייקונים
- אייקונים Base64 של אפליקציות Android (`data:image/png;base64,{icon}`)
- חיפוש חי לפי שם אפליקציה או package name
- גלילה פנימית (max-height: 220px)
- סגירה בלחיצה על backdrop, focus אוטומטי לשדה חיפוש (`{@attach}`)
- `$bindable` ל-value — תמיכה ב-`bind:value`

**5. רשימת אחרונות**

- עד 5 אחרונות נשמרות ב-`localStorage` (key: `fully-kiosk-recent-apps`)
- נטענות ב-`loadFromStorage()` — קיימות גם לפני טעינת רשימת כל האפליקציות
- מוצגות כשורת אייקונים 32px מעל ה-dropdown
- לחיצה על אחרון מפעיל ישירות ומקדם אותו בראש הרשימה

**6. הרחבת `fullyKioskClient.ts`**

- נוספו: `screenOn`, `screenOff`, `toForeground`, `toBackground`, `startApplication`, `loadStartUrl`, `getAppsList` (cmd=`manageApps`)

**7. טיפוסים ב-`fullyKioskTypes.ts`**

- נוסף `AppListItem` (icon, label, package, version, versionCode)
- נוסף `RecentApp` (package, label, icon)

#### החלטות ארכיטקטורה

- **Dropdown מותאם במקום `<select>`**: `<option>` לא תומך בתמונות — dropdown בנוי בידנו מאפשר הצגת אייקונים Base64
- **`{@attach}` במקום `use:action`**: Svelte 5.36+ מציע attachments כ-pattern מועדף לפעולות חד-פעמיות על DOM elements
- **RecentApp נפרד מ-AppListItem**: שמירת רק 3 שדות (package/label/icon) ב-localStorage במקום הטיפוס המלא — חוסך version/versionCode שאינם רלוונטיים

## 2026-02-26 22:00

### תיעוד 4 משימות חדשות בדף הסטטוס

עדכון `docs/features-status.md` בתיעוד משימות ורעיונות שנאספו — ללא כתיבת קוד.

#### מה בוצע?

**1. באגים חדשים שנוספו**

- **גרירה במסך טאצ'**: תועד שממשק ה-Drag & Drop לא עובד במסכי מגע, נדרשת ספריית touch אמינה.
- **הפעלת קיוסק מממשק הניהול**: תועדה בעיה שלאחר הפעלת קיוסק האפליקציה מתאפסת ולא ברור כיצד לחזור לרשימת המשימות.

**2. פיצ'רים מתוכננים חדשים**

- **משימה לסימון לפני ביצוע**: פיצ'ר לסמן משימה *לפני* ביצועה (למשל: "הולכים לישון"). נוסף סקשן `📋 סוגי משימות` חדש.
- **ממשק ניהול קיוסק מדף ווב ציבורי**: רעיון לנהל קיוסק דרך דף HTTPS ציבורי שמתקשר עם הלוקאל, בהתבסס על הקלה בהגבלות HTTPS→HTTP.
...
עיצוב מחדש של קומפוננטת סרגל הכלים (TaskActionBar) במצב עריכה תוך שימוש במחלקות Tailwind במקום בלוק Style רגיל, והוספת אפקטים מודרניים של ריחוף (Hover), Glassmorphism, ואייקוני SVG.

#### מה בוצע?

**1. ממשק משתמש (UI)**

- עיצוב הקונטיינר בצורת קפסולה (Pill) עם רקע חצי שקוף ואפקט טשטוש (backdrop-blur) להשתלבות טבעית מעל כרטיסי המשימות.
- החלפת האימוג'י הישנים באייקוני SVG מודרניים וברורים יותר.
- הוספת קווי הפרדה דקים עדינים בין החצים של התנועה לאייקונים של הפעולות סביב הפריט.
- הוספת אנימציות ריחוף (Hover) לכפתורים - גדילה קלה, שינוי צבע הדרגתי, והוספת צל ממוקד לפי סוג הפעולה (עריכה, קריטית, מידע).

**2. טכנולוגיה (CSS ו-Tailwind)**

- הסרת כל בלוק ה-`<style>` והעברת העיצוב לשימוש במחלקות המובנות (Utility classes) של Tailwind.
- חיווט (Binding) של מחלקות Tailwind משותפות לכפתורים לתוך משתנה `baseBtn` כדי לשמור על קוד נקי ותחזוקתי (DRY).
- שימוש ב-CSS Variables הקיימים של הפרויקט (`var(--primary)`, `var(--danger)` וכו') בתוך Tailwind כדי לשמור על תאימות מלאה לסוגי ערכות הנושא (Themes) הקיימים.
