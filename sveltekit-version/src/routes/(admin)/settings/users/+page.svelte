...
<div class="header-row">
    ...
    <Button size="sm" onclick={() => openUserModal()}>{TEXTS.NEW_USER}</Button>
</div>
    ...
    {#each userStore.users as user (user.id)}
        <Card class="user-card max-w-[280px]">
            ...
            <div class="user-details">
                <h3>{user.name}</h3>
                <Badge>{user.gender === 'boy' ? TEXTS.BOY : TEXTS.GIRL}</Badge>
            </div>
            <div class="user-actions">
                <IconButton title={TEXTS.EDIT} aria-label={TEXTS.EDIT} onclick={() => openUserModal(user)}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/></svg>
                </IconButton>
                <IconButton tone="danger" title={TEXTS.DELETE} aria-label={TEXTS.DELETE} onclick={() => deleteUser(user.id)}>
...
<ModalShell open={isUserModalOpen} onClose={() => (isUserModalOpen = false)} contentClass="max-w-[450px]">
    <h3 class="text-center text-2xl mb-8 text-slate-800">{editingUser ? TEXTS.EDIT_USER : TEXTS.NEW_USER}</h3>
    <form onsubmit={(e) => { e.preventDefault(); saveUser(); }}>
        <div class="form-group">
            <label for="user-name-input">{TEXTS.NAME}:</label>
            <TextInput id="user-name-input" bind:value={userForm.name} required />
        </div>
        <div class="form-group">
        ...
        <div class="modal-actions">
            <Button type="button" variant="secondary" onclick={() => (isUserModalOpen = false)}>{TEXTS.CANCEL}</Button>
