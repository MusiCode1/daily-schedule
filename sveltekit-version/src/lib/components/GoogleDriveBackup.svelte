<script lang="ts">
	...
	function handleClientIdSave() {
		...
		if (backupController.isConnected) {
			alert(TEXTS.APPLY_CLIENT_ID_RECONNECT);
		}
	}
			...
			<div class="connect-section">
				...
				<div class="advanced-settings">
					...
					>
						...
						<input
							type="checkbox"
							bind:checked={backupController.useRedirectMode}
							onchange={() => backupController.saveLocalSettings()}
						/>
					</label>
			...
			<div class="status-section" in:fade>
				<div class="user-info">
					{#if backupController.userInfo?.photoLink}
						<img src={backupController.userInfo.photoLink} alt={TEXTS.AVATAR} class="g-avatar" />
					{/if}
					<div>
						<span class="label">{TEXTS.CONNECTED_AS}</span>
						<span class="value"
							>{backupController.userInfo?.displayName ||
								backupController.userInfo?.emailAddress ||
				...
				<div class="actions-row">
					<button class="btn-outline" onclick={() => backupController.signOut()}
						>{TEXTS.DISCONNECT_DRIVE}</button
					>
					<button
						class="btn-primary"
						onclick={handleBackup}
						disabled={backupController.status === 'backing_up'}
					>
						{backupController.status === 'backing_up' ? '...' : TEXTS.BACKUP_NOW}
					</button>
					<button
						class="btn-outline"
						onclick={() => backupController.downloadLocalBackup()}
						title={TEXTS.SAVE_BACKUP_LOCAL_TITLE}
					>
						📥
					</button>
				</div>
				...
				<label class="toggle-row">
					<input
						type="checkbox"
						bind:checked={backupController.isAutoBackupEnabled}
						onchange={() => backupController.saveLocalSettings()}
					/>
				...
				{#if backupController.lastBackupTime}
					<div class="last-backup">
						{TEXTS.LAST_BACKUP}
						{backupController.lastBackupTime.toLocaleString('he-IL')}
					</div>
				{/if}
...
{#if isRestoreModalOpen}
	...
	<div
		class="modal-overlay"
		onclick={(e) => e.target === e.currentTarget && (isRestoreModalOpen = false)}
	>
				...
				{#each restoreFiles as file}
					<div style="display: flex; align-items: stretch;">
						...
						<button
							...
							onclick={(e) => {
								e.stopPropagation();
								backupController.downloadRemoteBackup(file.id);
							}}
							title={TEXTS.DOWNLOAD_FILE_TITLE}
						>
							⬇️
						</button>
					</div>
		...
		<div class="modal-card warning">
			...
			<div class="conflict-comparison">
				...
				<div class="conflict-option local">
					...
					<span
						>{backupController.conflictState.localTime?.toLocaleString('he-IL') || TEXTS.UNKNOWN}</span
					>
				</div>
			</div>
			...
			<div class="modal-actions" style="justify-content: space-between; margin-top: 1rem;">
				<button
					class="btn-conflict-remote"
					onclick={() => backupController.resolveConflict('remote')}
				>
					{TEXTS.KEEP_REMOTE}
				</button>
				<button
					class="btn-conflict-local"
					onclick={() => backupController.resolveConflict('local')}
				>
					{TEXTS.KEEP_LOCAL}
				</button>
			</div>
